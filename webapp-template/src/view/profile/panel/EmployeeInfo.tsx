...
import { yearsOfService } from "@root/src/utils/utils";
import { Edit, User } from "lucide-react";

function EmployeeInfo() {
  const user = useAppSelector((state: RootState) => state.user);
  const employee_info = useAppSelector((state: RootState) => state.employee.employee);
  ...
  console.log(employee_info?.startDate);
  console.log(yearsOfService(employee_info?.startDate));
      ...
      <div className="flex p-1">
        ...
        <div className="flex flex-col items-end gap-3 justify-end text-st-200 whitespace-nowrap p-r">
          ...
          <div className="flex items-center justify-center">
            <p>Service Time&nbsp;</p>{" "}
            <p className="h6">{`: ${yearsOfService(employee_info?.startDate)} years`}</p>
          </div>
